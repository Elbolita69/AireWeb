...
function saveProject() {
  ...
  // Simular almacenamiento de archivos (ahora con placeholders si no hay archivo)
  project.documentos.kmz = kmzFile
    ? { nombre: kmzFile.name, tipo: kmzFile.type, tamaño: kmzFile.size }
    : { nombre: "KMZ #1", tipo: "application/vnd.google-earth.kmz", tamaño: 0 }

  project.documentos.dwg = dwgFile
    ? { nombre: dwgFile.name, tipo: dwgFile.type, tamaño: dwgFile.size }
    : { nombre: "DWG #1", tipo: "application/acad", tamaño: 0 }

  project.documentos.plano = planoFile
    ? { nombre: planoFile.name, tipo: planoFile.type, tamaño: planoFile.size }
    : { nombre: "PDF #1", tipo: "application/pdf", tamaño: 0 }

  project.documentos.matricula = matriculaFile
    ? { nombre: matriculaFile.name, tipo: matriculaFile.type, tamaño: matriculaFile.size }
    : { nombre: "PDF #2", tipo: "application/pdf", tamaño: 0 }

  project.documentos.cc = ccFile
    ? { nombre: ccFile.name, tipo: ccFile.type, tamaño: ccFile.size }
    : { nombre: "PDF #3", tipo: "application/pdf", tamaño: 0 }

  project.documentos.formulario = formularioFile
    ? { nombre: formularioFile.name, tipo: formularioFile.type, tamaño: formularioFile.size }
    : { nombre: "Excel #1", tipo: "application/vnd.ms-excel", tamaño: 0 }
  ...
  Storage.createNotification({
    ...
    tipo: projectId ? "proyecto_actualizado" : "proyecto_creado",
    mensaje: projectId
      ? `Has actualizado el proyecto "${project.nombre}" con ID ${project.id}.`
      : `Has creado un nuevo proyecto "${project.nombre}" con ID ${project.id}.`,
    fechaCreacion: new Date().toISOString(),
    leido: false,
  })
...
function viewProject(projectId) {
  ...
  if (project.documentos.plano) {
    document.getElementById("detail-plano-link").href = "#"
    document.getElementById("detail-plano-link").onclick = () =>
      alert("Descargando archivo PDF: " + project.documentos.plano.nombre)
  }
