document.addEventListener("DOMContentLoaded", () => {
    ...
    <div class="col-md-4 mb-4">
      <div class="card h-100">
        <div class="card-header d-flex justify-content-between align-items-center">
          ...
          <span class="badge ${estadoClase}">${Storage.getStatusDisplayName(proyecto.estado)}</span>
        </div>
        <div class="card-body">
          <p class="card-text">${proyecto.descripcion || "Sin descripción"}</p>
          <p class="card-text"><strong>Departamento:</strong> ${proyecto.sector || "No asignado"}</p>
  ...
  function cargarDatosPerfilOriginal() {
    // Mostrar información del usuario
    ...
    // Mostrar sector si es brigada
    if (loggedUser.rol === "brigada") {
      document.getElementById("perfilSector").style.display = "block"
      document.getElementById("perfilSector").textContent = `Sector: ${loggedUser.sector}`
    } else {
      document.getElementById("perfilSector").style.display = "none"
    }

    // Mostrar cargo
    document.getElementById("perfilCargo").textContent = `Cargo: ${loggedUser.cargo}`

    // Llenar formulario
    document.getElementById("perfilUsuario").value = loggedUser.usuario
  ...
  // Modificar la función cargarPerfil para mostrar información completa
  function cargarDatosPerfil() {
    ...
    const perfilSectorContainer = document.getElementById("perfilSectorContainer")
    if (loggedUser.rol === "brigada" && loggedUser.sector) {
      perfilSectorContainer.style.display = "block"
      document.getElementById("perfilSector").textContent = loggedUser.sector
    } else {
      perfilSectorContainer.style.display = "none"
    }

    // Mostrar cargo
    const perfilCargoContainer = document.getElementById("perfilCargoContainer")
    if (loggedUser.cargo) {
      perfilCargoContainer.style.display = "block"
      document.getElementById("perfilCargo").textContent = loggedUser.cargo
    } else {
      perfilCargoContainer.style.display = "none"
    }
  }
